...
#'
#' @param DE_genes_pw List of differentially expressed genes as output given by diff_limma_pw(). Rownames should be gene names (SMYBOLS).
...
#' @examples
#' pathview_func(DE_genes_pw, pathway.id = "04620", out.suffix = "DE_TolLPS") #Toll-like-receptor signaling
#' @export
pathview_func <- function(DE_genes_pw, pathway.id, out.suffix){

  DE_genes_pw$entrez <- AnnotationDbi::mapIds(org.Hs.eg.db,
  ...
  pathview::pathview(gene.data = vals, pathway.id = pathway.id, gene.idtype = "ENTREZ",
           species = "hsa", out.suffix = out.suffix, title = out.suffix)
