...
pca_plot_enrich <- function(expmatrix, groups, PCs = c(1,2), main = "PCA plot", percentage = 2.5, inputType = "geneSymbol", org = "Hs", projectfolder = getwd()){
  ...
  # Plot PCA
  grDevices::pdf(file=file.path(projectfolder, "pcaGoPromoter", paste0("PCAplot_PCs_", PCs[1], "_", PCs[2], ".pdf")))
  ...
  for (PC in PCs){
    ...
    utils::write.table(TFtableNeg[which(TFtableNeg$pValue < 0.05),], file.path(projectfolder, "pcaGoPromoter", paste0("TFtableNeg_PC_", PC, ".txt")), col.names = T, row.names = F, sep = "\t")
    cat("\n-------------------------\n", "Table of enriched TFs of negative loadings of PC", PC, "saved to", file.path(projectfolder, paste0("pcaGoPromoter/TFtableNeg_PC_", PC, ".txt")), "\n-------------------------\n")
    ...
    utils::write.table(TFtablePos[which(TFtablePos$pValue < 0.05),], file.path(projectfolder, "pcaGoPromoter", paste0("TFtablePos_PC_", PC, ".txt")), col.names = T, row.names = F, sep = "\t")
    cat("\n-------------------------\n", "Table of enriched TFs of positive loadings of PC", PC, "saved to", file.path(projectfolder, paste0("pcaGoPromoter/TFtablePos_PC_", PC, ".txt")), "\n-------------------------\n")
    ...
    utils::write.table(sigGOTermsPos[which(sigGOTermsPos$pValue < 0.05),], file.path(projectfolder, "pcaGoPromoter", paste0("sigGOTermsPos_PC_", PC, ".txt")), col.names = T, row.names = F, sep = "\t")
    cat("\n-------------------------\n", "Table of enriched GO terms of positive loadings of PC", PC, "saved to", file.path(projectfolder, paste0("pcaGoPromoter/sigGOTermsPos_PC_", PC, ".txt")), "\n-------------------------\n")
    ...
    utils::write.table(sigGOTermsNeg[which(sigGOTermsNeg$pValue < 0.05),], file.path(projectfolder, "pcaGoPromoter", paste0("sigGOTermsNeg_PC_", PC, ".txt")), col.names = T, row.names = F, sep = "\t")
    cat("\n-------------------------\n", "Table of enriched GO terms of negative loadings of PC", PC, "saved to", file.path(projectfolder, paste0("pcaGoPromoter/sigGOTermsNeg_PC_", PC, ".txt")), "\n-------------------------\n")
  }
